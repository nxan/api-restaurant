...
module.exports = {
    ...
    findOrderByDeskEnd: (req, res, next) => {
        ...
        let sql = "SELECT BH_tbd_BanHangLyLich.*, tbl_Ban.TenBan FROM BH_tbd_BanHangLyLich JOIN tbl_Ban ON tbl_Ban.MaBan = BH_tbd_BanHangLyLich.MaBan WHERE BH_tbd_BanHangLyLich.MaBan = " + deskId + " AND GIOVAO = '" + timeEnd + "'";
        console.log(sql);
        let query = db.query(sql)
            .then(results => {
                console.log(results);
